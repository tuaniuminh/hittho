/**
 * Dịch vụ kiểm tra và tải bản cập nhật trực tiếp trong ứng dụng (In-App OTA Updater)
 * Kết nối GitHub Releases API, tải file IPA hiển thị MB/s và mở TrollStore qua Share Sheet.
 */
...
export const checkForUpdate = async (currentVersion) => {
  try {
    ...
    const repo = settings.repoUrl || 'tuaniuminh/hittho';

    // Thêm timestamp để chống WebKit / iOS cache dữ liệu cũ
    const url = `https://api.github.com/repos/${repo}/releases/latest?t=${Date.now()}`;
...
export const downloadIPAInApp = async (downloadUrl, onProgressCallback) => {
  ...
  // Fallback mở Safari tải trực tiếp nếu chạy trên môi trường Web
  window.open(downloadUrl, '_blank');
  return { success: true, fallback: true };
};
